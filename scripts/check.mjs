...
try {
  execFileSync(process.execPath, [join(root, "scripts/production-submission-smoke.mjs")], { stdio: "pipe" });
} catch (error) {
  failures.push(`Production-отправка заявки: ${error.stderr?.toString() || error.message}`);
}
...
console.log("— production-отправка заявки через API");
